...
import { PetitionService, extractPageContent } from '../../services/petition.service';
import { AuthService } from '../../services/auth.service';
import { PetitionCategory, PetitionResponse, getCategoryLabel } from '../../models/petition.model';
    ...
    <div class="space-y-8">
      ...
      <div class="evo-card p-5 space-y-4">
        ...
        <div class="flex items-center gap-2 overflow-x-auto pb-1 text-xs font-semibold">
          ...
          <button
            (click)="selectCategory('ADMINISTRATIE_PUBLICA')"
            [class]="selectedCategory === 'ADMINISTRATIE_PUBLICA' ? 'bg-evo-cobalt text-white' : 'bg-slate-100 text-evo-text-muted hover:bg-slate-200'"
            class="px-3 py-1.5 rounded-lg transition-colors whitespace-nowrap"
          >
          ...
          <button
            ...
            [class]="selectedCategory === 'SOCIAL' ? 'bg-evo-cobalt text-white' : 'bg-slate-100 text-evo-text-muted hover:bg-slate-200'"
            class="px-3 py-1.5 rounded-lg transition-colors whitespace-nowrap"
          >
            Protecție socială
          </button>
          <button
            (click)="selectCategory('EDUCATIE')"
            [class]="selectedCategory === 'EDUCATIE' ? 'bg-evo-cobalt text-white' : 'bg-slate-100 text-evo-text-muted hover:bg-slate-200'"
            class="px-3 py-1.5 rounded-lg transition-colors whitespace-nowrap"
          >
            Educație
          </button>
            ...
            <div class="evo-card p-6 flex flex-col justify-between space-y-5">
              <div class="space-y-3">
                ...
                @if (item.targetAuthority) {
                  <div class="flex items-center gap-1.5 text-xs text-evo-cobalt font-semibold">
                    <svg class="w-3.5 h-3.5 text-evo-cobalt shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"/></svg>
                    <span>Către: {{ item.targetAuthority }}</span>
                  </div>
                }
      ...
      <!-- Modal Confirmare Semnare Petiție -->
      @if (showConfirmModal && confirmingInitiative) {
          ...
          <div class="bg-white rounded-3xl max-w-md w-full p-6 sm:p-8 shadow-2xl space-y-6 border border-evo-border" role="dialog" aria-modal="true">
            ...
            <div class="bg-slate-50 border border-evo-border p-4 rounded-2xl space-y-2">
              ...
              <div class="font-bold text-evo-navy text-sm leading-snug line-clamp-2">
                {{ confirmingInitiative.title }}
              </div>
              <div class="text-[11px] font-mono text-evo-cobalt">
                #{{ confirmingInitiative.trackingNumber }}
              </div>
            </div>
            ...
            <div class="flex gap-3 pt-2">
              ...
              <button
                type="button"
                (click)="confirmSign()"
                [disabled]="signingId === confirmingInitiative.id"
              ...
              >
                @if (signingId === confirmingInitiative.id) {
...
export class PublicInitiativesComponent implements OnInit {
  ...
  confirmingInitiative: PetitionResponse | null = null;
  ...
  loadInitiatives() {
    ...
    this.petitionService.getPublicInitiatives(this.searchTerm, this.selectedCategory).subscribe({
      next: (res) => {
        this.initiatives = extractPageContent(res);
  ...
  promptSign(item: PetitionResponse) {
    if (!this.authService.isAuthenticated()) {
      this.router.navigate(['/login']);
      return;
    }
    this.confirmingInitiative = item;
    this.showConfirmModal = true;
  }

  cancelSign() {
    this.showConfirmModal = false;
    this.confirmingInitiative = null;
  }

  confirmSign() {
    if (!this.confirmingInitiative) return;
    const item = this.confirmingInitiative;
